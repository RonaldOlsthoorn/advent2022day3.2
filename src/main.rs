...
fn main() {
    ...
    let mut letterBins = Vec::new();
    letterBins.push(HashSet::new());
    letterBins.push(HashSet::new());
    letterBins.push(HashSet::new());
    ...
    for (i, line) in reader.lines().enumerate() {
        ...
        for c in l.chars() {
            if period == 0 {
                letterBins[0].insert(c);
            } else if letterBins[period -1].contains(&c){
                letterBins[period].insert(c);
            }
        }

        if period == 2 {
            println!("found: {}", letterBins[2].iter().next().unwrap());
            score += scoreMap.get(letterBins[2].iter().next().unwrap()).unwrap();
